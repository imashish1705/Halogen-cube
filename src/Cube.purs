...
oneDegInRadD :: Angle
oneDegInRadD = 0.01745329255

oneDegInRadV :: Angle
oneDegInRadV = 0.01745329255

oneDegInRadR :: Angle
oneDegInRadR = -0.01745329255
...
--increase speed
accelerateByV :: Number
accelerateByV = oneDegInRadV * 60.0

--reverse speed 
accelerateByR :: Number
accelerateByR = oneDegInRadR * 50.0

--decreasing spedd
accelerateByD :: Number
accelerateByD = oneDegInRadD * 30.0
...
data Query a  
  ...
  | DecAngVelocity Axis a
  | InncAngVelocity Axis a
  | RevAngVelocity Axis a
  ...
  where
    ...
    eval = case _ of
      ...
      IncAngVelocity axis next -> do
        ...
        pure next
      DecAngVelocity axis next -> do
        cube <- H.get
        let {xa, ya, za} = cube.angVel
        H.modify
          (\c ->
            case axis of
              X -> c { angVel { xa = xa - accelerateByD } }
              Y -> c { angVel { ya = ya - accelerateByD } }
              Z -> c { angVel { za = za - accelerateByD } }
          )
        pure next
      InncAngVelocity axis next -> do
        cube <- H.get
        let {xa, ya, za} = cube.angVel
        H.modify
          (\c ->
            case axis of
              X -> c { angVel { xa = xa + accelerateByV } }
              Y -> c { angVel { ya = ya + accelerateByV } }
              Z -> c { angVel { za = za + accelerateByV } }
          )
        pure next
      RevAngVelocity axis next -> do
        cube <- H.get
        let {xa, ya, za} = cube.angVel
        H.modify
          (\c ->
...
renderView state = let
  ...
  in
    HH.div [] $
      ...
      , renderButton "reverse" (RevAngVelocity Y)
      , renderButton "val++" (InncAngVelocity X)
      , renderButton "val--" (DecAngVelocity X)
      , renderButton "add" (IncAngVelocity Z)
